...
{
    public partial class XAxisAggregationForm : Form
    {
        ...
        public XAxisAggregationForm()
        {
            InitializeComponent();
        }

        public XAxisAggregationForm(IQueryAsParameterService ISimpleAggregationService)
        {
        ...
        private void UpdateQueryTextBox()
        {
            ...
            string finalQuery = string.Format(@"SELECT {0} as ""X AXIS LABEL"", {1}({2}) AS ""X AXIS VALUE"" FROM ""{3}"".""{4}"".""{5}"" GROUP BY {6}", XAxisLabel, Operator, Column, Database, Schema, Table, XAxisLabel);
